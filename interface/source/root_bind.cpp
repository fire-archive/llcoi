...
{
  Ogre::Root* root = reinterpret_cast<Ogre::Root*>(root_handle);
  Ogre::NameValuePairList* p = reinterpret_cast<Ogre::NameValuePairList*>(params);
  Ogre::RenderWindow* window = root->createRenderWindow(name, width, height, full_screen, p);
  return reinterpret_cast<RenderWindowHandle>(window);
}
